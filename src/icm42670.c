 ...
 *******************************************************************************/
static esp_err_t icm42670_write(icm42670_handle_t sensor,
                                const uint8_t reg_start_addr,
                                const uint8_t *data_buf,
                                const uint8_t data_len);
static esp_err_t icm42670_read(icm42670_handle_t sensor,
                               const uint8_t reg_start_addr, uint8_t *data_buf,
                               const uint8_t data_len);

static esp_err_t icm42670_get_raw_value(icm42670_handle_t sensor, uint8_t reg,
                                        icm42670_raw_value_t *value);
...
icm42670_handle_t icm42670_create(i2c_port_t port, const uint8_t dev_addr) {
  icm42670_dev_t *sensor = (icm42670_dev_t *)heap_caps_calloc(
      1, sizeof(icm42670_dev_t), MALLOC_CAP_DEFAULT);
  sensor->bus = port;
  ...
  ESP_LOGI(TAG, "Found device %s, ID: 0x%02x",
           (dev_id == ICM42607_ID ? "ICM42607" : "ICM42670"), dev_id);
...
esp_err_t icm42670_config(icm42670_handle_t sensor,
                          const icm42670_cfg_t *config) {
  uint8_t data[2];
...
esp_err_t icm42670_acce_set_pwr(icm42670_handle_t sensor,
                                icm42670_acce_pwr_t state) {
  esp_err_t ret = ESP_FAIL;
  uint8_t data;
...
esp_err_t icm42670_gyro_set_pwr(icm42670_handle_t sensor,
                                icm42670_gyro_pwr_t state) {
  esp_err_t ret = ESP_FAIL;
  uint8_t data;
...
esp_err_t icm42670_get_acce_sensitivity(icm42670_handle_t sensor,
                                        float *sensitivity) {
  esp_err_t ret = ESP_FAIL;
  uint8_t acce_fs;
...
esp_err_t icm42670_get_gyro_sensitivity(icm42670_handle_t sensor,
                                        float *sensitivity) {
  esp_err_t ret = ESP_FAIL;
  uint8_t gyro_fs;
...
esp_err_t icm42670_get_temp_raw_value(icm42670_handle_t sensor,
                                      uint16_t *value) {
  esp_err_t ret = ESP_FAIL;
  uint8_t data[2];
...
esp_err_t icm42670_get_acce_raw_value(icm42670_handle_t sensor,
                                      icm42670_raw_value_t *value) {
  return icm42670_get_raw_value(sensor, ICM42670_ACCEL_DATA, value);
}

esp_err_t icm42670_get_gyro_raw_value(icm42670_handle_t sensor,
                                      icm42670_raw_value_t *value) {
  return icm42670_get_raw_value(sensor, ICM42670_GYRO_DATA, value);
}

esp_err_t icm42670_get_acce_value(icm42670_handle_t sensor,
...
esp_err_t icm42670_get_gyro_value(icm42670_handle_t sensor,
                                  icm42670_value_t *value) {
  esp_err_t ret;
  float sensitivity;
  icm42670_raw_value_t raw_value;
...
static esp_err_t icm42670_write(icm42670_handle_t sensor,
  ...
  assert(ESP_OK == ret);
  ret = i2c_master_write_byte(cmd, (sens->dev_addr << 1) | I2C_MASTER_WRITE,
                              true);
  assert(ESP_OK == ret);
...
static esp_err_t icm42670_read(icm42670_handle_t sensor,
  ...
  return i2c_master_write_read_device(sens->bus, sens->dev_addr, reg_buff,
                                      sizeof(reg_buff), data_buf, data_len,
                                      1000 / portTICK_PERIOD_MS);
}

esp_err_t icm42670_complimentory_filter(
    icm42670_handle_t sensor, const icm42670_value_t *const acce_value,
